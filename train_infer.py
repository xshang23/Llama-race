...
def load_model_and_tokenizer(model_name = "mistralai/Mistral-7B-v0.1"):
    ...
    tokenizer = AutoTokenizer.from_pretrained(model_name, trust_remote_code=True, token='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')
...
def train(dir='data',train_shuffle=False):
    ...
    train_data, eval_data = load_balanced_training_data(dir,train_shuffle,90000,10000)
    # load test data
    test_data, y_true = load_test_data(dir, frac=0.01)
    shuffle_test, y_true1 = load_test_data(dir, frac=0.01, shuffle=True)

    with open('statistics_balanced_data_mis.txt', 'w') as f:
    ...
    with open('bal_test0411_true.txt', 'w') as f:
        for i in range(len(y_true)):
            f.write(f'{y_true[i]}\n')

    with open('bal_test0411_true_shuffle.txt', 'w') as f:
        for i in range(len(y_true1)):
            f.write(f'{y_true1[i]}\n')

    with open('bal_test0411_unsh_res.txt', 'w') as f:
        for i in range(len(y_pred)):
            f.write(f'{y_pred[i]}\n')

    with open('bal_test0411_sh_res.txt', 'w') as f:
        for i in range(len(y_pred1)):
            f.write(f'{y_pred1[i]}\n')

    evaluate(y_true, y_pred, 'bal_test0411_unsh_report.txt')
    evaluate(y_true, y_pred1, 'bal_test0411_sh_report.txt')
