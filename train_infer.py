...
def train(dir='data',train_shuffle=False):
    ...
    with open('bal_test0409_true.txt', 'w') as f:
        for i in range(len(y_true)):
            f.write(f'{y_true[i]}\n')

    with open('bal_test0409_true_shuffle.txt', 'w') as f:
        for i in range(len(y_true1)):
            f.write(f'{y_true1[i]}\n')

    with open('bal_test0409_unsh_res.txt', 'w') as f:
        for i in range(len(y_pred)):
            f.write(f'{y_pred[i]}\n')

    with open('bal_test0409_sh_res.txt', 'w') as f:
        for i in range(len(y_pred1)):
            f.write(f'{y_pred1[i]}\n')

    evaluate(y_true, y_pred, file_name='bal_test0409_720K_unsh_report.txt')
    evaluate(y_true1, y_pred1, file_name='bal_test0409_720K_sh_report.txt')
