...
def load_test_data(dir='data', frac=1, shuffle=False, batch_size=16):
    """
    ...
    test_df1 = pd.DataFrame(test_df.apply(lambda row: generate_prompt(row, shuffle=shuffle), axis=1), columns=["name"])
    y_test_true = [race2num[category] for category in test_df.label.values]
    test_df = pd.DataFrame(test_df.apply(lambda row: generate_test_prompt(row, shuffle=shuffle), axis=1), columns=["name"])
    test_data = MyDataset(test_df1, y_test_true)
    test_loader = DataLoader(test_data, batch_size=batch_size, shuffle=False, collate_fn=custom_collate)
    del test_df1
    return test_df, y_true, test_loader
